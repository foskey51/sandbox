...
const ProfilePage = () => {
  ...
  const [editMode, setEditMode] = useState({ fullName: false, email: false, bio: false, username: false });
  const [tempValues, setTempValues] = useState({ fullName: '', email: '', bio: '', username: '' });
  ...
  const formatImageUrl = (base64Str, mimeType = 'image/jpeg') => {
    ...
    return isValidBase64(base64Str) ? `data:${mimeType};base64,${base64Str}` : '';
  };
    ...
    api
      ...
      .then((res) => {
        ...
        setProfile({
          ...
          profileImage: formatImageUrl(data.profileImage, 'image/jpeg') || '',
        });
        setTempValues({
          fullName: data.fullName || '',
          email: data.email || '',
  ...
  const handleUpdate = async (field) => {
    const value = tempValues[field] || profile[field] || '';

    if ((field === 'fullName' && value.length > 50) ||
      (field === 'bio' && value.length > 160) ||
      (field === 'username' && value.length > 30)) { // username max length
      setErrors((prev) => ({ ...prev, [field]: `Must be ${field === 'fullName' ? 50 : field === 'bio' ? 160 : 30} characters or less` }));
      return;
    }
    ...
    try {
      const response = await api.patch(`/api/v1/profile/update`, payload, { headers: { 'Content-Type': 'application/json' } });
      const data = response.data || {};
      setProfile((prev) => ({
        ...prev,
        [field]: tempValues[field] ?? profile[field],
        profileImage: formatImageUrl(data.profileImage, 'image/jpeg') || prev.profileImage,
      }));
              ...
              <div className="flex flex-col items-center space-y-4">
                <div className="relative">
                  <img
                    ...
                    onError={(e) => { e.target.src = 'https://cdn.pixabay.com/photo/2023/02/18/11/00/icon-7797704_1280.png'; }}
                  />
                  ...
                  {errors.profilePic && <p className="text-red-500 text-sm mt-2 text-center">{errors.profilePic}</p>}
                </div>
                <h1 className="text-2xl font-bold text-center">{profile.fullName || 'New User'}</h1>
                <p className="text-gray-600 dark:text-gray-400">@{profile.username || 'Not set'}</p>
                <div className="w-full mt-4 text-center">
                  {editMode.bio ? (
                    <div>
                      <textarea value={tempValues.bio || ''} onChange={(e) => handleInputChange(e, 'bio')} style={{ resize: "none", overflow: "auto" }} className="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" rows="3" maxLength={160} />
                      <div className="flex justify-end mt-2">
                        <button onClick={() => handleUpdate('bio')} className="flex items-center bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
                          <IconCheck size={20} /><span className="ml-1">Save</span>
                        </button>
                      </div>
                    ...
                    <div className="flex justify-center items-center space-x-4">
                      ...
                      <button onClick={() => toggleEditMode('bio')} className="text-blue-600 hover:text-blue-800">
                        <IconEdit size={20} />
                      </button>
                    </div>
                  )}
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => toggleEditMode('fullName')} className="text-blue-600 hover:text-blue-800">
                  {editMode.fullName ? <IconX size={20} /> : <IconEdit size={20} />}
                </button>
              </div>
              {editMode.fullName ? (
                <div className="mt-2">
                  <input type="text" value={tempValues.fullName || ''} onChange={(e) => handleInputChange(e, 'fullName')} className="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" maxLength={50} />
                  <div className="flex justify-end mt-2">
                    <button onClick={() => handleUpdate('fullName')} className="flex items-center bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
                      <IconCheck size={20} /><span className="ml-1">Save</span>
                    </button>
                  </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => toggleEditMode('email')} className="text-blue-600 hover:text-blue-800">
                  {editMode.email ? <IconX size={20} /> : <IconEdit size={20} />}
                </button>
              </div>
              {editMode.email ? (
                <div className="mt-2">
                  <input type="email" value={tempValues.email || ''} onChange={(e) => handleInputChange(e, 'email')} className="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" />
                  <div className="flex justify-end mt-2">
                    <button onClick={() => handleUpdate('email')} className="flex items-center bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
                      <IconCheck size={20} /><span className="ml-1">Save</span>
                    </button>
                  </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => toggleEditMode('username')} className="text-blue-600 hover:text-blue-800">
                  {editMode.username ? <IconX size={20} /> : <IconEdit size={20} />}
                </button>
              </div>
              {editMode.username ? (
                <div className="mt-2">
                  ...
                  />
                  <div className="flex justify-end mt-2">
                    <button onClick={() => handleUpdate('username')} className="flex items-center bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
                      <IconCheck size={20} /><span className="ml-1">Save</span>
                    </button>
                  </div>
